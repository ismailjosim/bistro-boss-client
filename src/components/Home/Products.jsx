...
const Products = () => {
  ...
  const filteredProducts = products.filter((product) => product.categoryId === activeTab);
    ...
    <div className="w-11/12 mx-auto py-20">
      ...
      <div className="tabs tabs-boxed flex justify-center items-center bg-transparent">
        {categories?.map((category, index) => {
          ...
          return (
            <button
              className={`tab tab-rounded-none  ${
                activeTab === categoryId
                  ? 'tab-active bold text-white uppercase font-semibold'
                  : 'normal'
              }`}
              key={_id}
