...
const Shop = () => {
	...
	const categories = useMemo(
		() => ['all', 'offered', 'dessert', 'pizza', 'salad', 'soup'],
		[],
	)
	...
	const allFood = menu
		.slice(0, 3)
		.map((item, index) => <ShopCard key={index} item={item} />)
	const offeredItem = menu
		.filter((item) => item.category === 'offered')
		.map((item, index) => <ShopCard key={index} item={item} />)
	const dessertItem = menu
		.filter((item) => item.category === 'dessert')
		.map((item, index) => <ShopCard key={index} item={item} />)
	const pizzaItem = menu
		.filter((item) => item.category === 'pizza')
		.map((item, index) => <ShopCard key={index} item={item} />)
	const saladItem = menu
	...
	return (
		<main>
			<PageHeading
				bannerImg={bannerImg}
				primary={'Our shop'}
				secondary={'would you like to try a dish?'}
			/>
			<div className=''>
				<Tabs
				...
				>
					<TabList className={'tabs flex justify-center items-center'}>
						...
						<Tab className={`tab tab-bordered text-lg font-semibold`}>
							Today's Offer
						</Tab>
						<Tab className={'tab tab-bordered text-lg font-semibold'}>
							Desserts
